...
export default function AdminSettingsPage() {
  ...
  const handleReorderEnvironments = async (newOrder) => {
    setEnvironments(newOrder);
    try {
      await authFetch(`${API_BASE_URL}/api/options/reorder`, {
        method: 'POST',
        body: JSON.stringify(newOrder.map((item) => item.id)),
      });
    } catch (_) {
  ...
  const handleReorderRoles = async (newOrder) => {
    setRoles(newOrder);
    try {
      await authFetch(`${API_BASE_URL}/api/options/reorder`, {
        method: 'POST',
