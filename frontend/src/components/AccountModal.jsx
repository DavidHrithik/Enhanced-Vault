...
export default function AccountModal({ open, onClose, onSubmit, initial, originPosition }) {
  ...
  const [form, setForm] = useState(
    initial ? { ...initial, password: initial.password || '' } : { ...defaultForm }
  );
  ...
  useEffect(() => {
    if (open) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setForm(initial ? { ...initial, password: initial.password || '' } : { ...defaultForm });
    } else {
      setForm({ ...defaultForm });
    }
  }, [open, initial]);
      ...
      >
        ...
        >
          {showContent ? (
            <>
              <button
                className="absolute top-3 right-4 text-white bg-gradient-to-r from-primary to-secondary rounded-full w-8 h-8 flex items-center justify-center shadow hover:from-secondary hover:to-primary focus:outline-none"
                onClick={onClose}
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <h2 className="text-2xl font-extrabold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-primary via-secondary to-surface drop-shadow-lg">
                ...
                <div>
                  ...
                  <input
                    name="username"
                    className="border border-blue-400/30 bg-surface/80 text-blue-200 px-3 py-2 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-secondary placeholder:text-blue-400 w-full"
                ...
                <div>
                  ...
                  {errors.environment && (
                    <div className="text-red-500 text-xs">{errors.environment}</div>
                  )}
                </div>
                <div>
                  <label className="block font-bold text-blue-200 flex items-center gap-1">
                    Account Owner
                    <span
                      title="OTP will be sent to the respective owners."
                      className="inline-block align-middle cursor-pointer"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="h-4 w-4 text-blue-400 inline"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
            ...
            <div className="flex flex-col items-center justify-center h-48 w-full">
              ...
              <span className="animate-spin rounded-full h-10 w-10 border-t-4 border-b-4 border-blue-400 border-opacity-40 border-t-primary border-b-[secondary]"></span>
            </div>
          )}
        </motion.div>
      </div>
