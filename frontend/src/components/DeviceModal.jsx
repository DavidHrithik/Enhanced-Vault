...
export default function DeviceModal({ open, onClose, onSubmit, originPosition }) {
  ...
  const fetchOwners = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const owners = [...new Set(data.map((d) => d.owner).filter((o) => o && o !== 'None'))];
        setUniqueOwners(owners.sort());
      }
    } catch (e) {
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <select
                    name="status"
                    className="border border-blue-400/30 bg-surface/80 text-blue-200 px-3 py-2 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-secondary w-full"
                    value={form.status}
                    onChange={handleChange}
                    disabled={loading}
                  >
